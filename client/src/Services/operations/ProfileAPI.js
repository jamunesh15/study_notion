...
export const getinstructorDashboard = async(token)=>{
  const toastId = toast.loading("Loading Instructor Dashboard...")
   let result = []
   ...
   try {
    ...
    console.log("INSTRUCTOR_DASHBOARD_API RESPONSE............", response)
    if (!response.data.success) {
      throw new Error(response.data.message)
    }

    result = response?.data?.courses
    console.log("INSTRUCTOR_DASHBOARD_API RESULT:", result)
    toast.success("Instructor Dashboard Loaded Successfully")
    
   } catch (error) {
     console.log("INSTRUCTOR_DASHBOARD_API_ERROR:", error)
